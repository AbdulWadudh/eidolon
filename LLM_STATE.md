...
4. **`bun --watch` does not see `packages/`.** The conductor runs with its cwd
   at `apps/conductor`, and `@eidolon/config` reaches it through a symlink in
   `node_modules` pointing out of that tree, which the watcher does not follow.
   Editing anything under `packages/` — a copy string, an `AuthorField`, a queue
   name — leaves the running conductor on the old value, and it answers with
   whatever the old code said: `Unknown field.` for a field added since it
   booted. **Restart the conductor after touching `packages/`.** Running it from
   the repo root would fix the watch and break `.env` loading, which is read
   from the cwd.
