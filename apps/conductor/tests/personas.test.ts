...
describe("the persona every character reads", () => {
  it("makes the first one the default, because nobody would read a reader with none", () => {
    const first = createPersona(READER, { name: "First" });
    const second = createPersona(READER, { name: "Second" });
  ...
  it("keeps exactly one default when another is chosen", () => {
    ...
    makeDefaultPersona(second.id, READER);
    const defaults = listPersonas(READER).filter((persona) => persona.isDefault);
  ...
  it("hands the default to whoever is left when the default is deleted", () => {
    ...
    makeDefaultPersona(second.id, READER);
    deletePersona(second.id, READER);

    expect(listPersonas(READER).find((persona) => persona.isDefault)?.id).toBe(first.id);
  });
});

describe("which persona a character reads", () => {
  it("reads the default when nothing is pinned", () => {
    const persona = createPersona(READER, { name: "Default one" });
    const character = freshCharacter("persona-reads-default");

    expect(personaForCharacter(character, READER)?.id).toBe(persona.id);
  });

  it("reads the pinned one instead, and leaves other characters alone", () => {
    const fallback = createPersona(READER, { name: "Everywhere" });
    const pinned = createPersona(READER, { name: "Just here" });

    const one = freshCharacter("persona-pinned");
    const other = freshCharacter("persona-unpinned");

    pinPersonaToCharacter(one, READER, pinned.id);

    expect(personaForCharacter(one, READER)?.id).toBe(pinned.id);
    expect(personaForCharacter(other, READER)?.id).toBe(fallback.id);
  });
  ...
  it("falls back to the default once the pin is cleared", () => {
    ...
    pinPersonaToCharacter(character, READER, pinned.id);
    pinPersonaToCharacter(character, READER, null);

    expect(personaForCharacter(character, READER)?.id).toBe(fallback.id);
  });
  ...
  it("refuses to pin a persona belonging to someone else", () => {
    ...
    expect(pinPersonaToCharacter(character, READER, theirs.id)).toBe(false);
    deletePersona(theirs.id, "user:someone-else");
  });
});

describe("what the character is told about the reader", () => {
  it("carries every part the reader filled in", () => {
    createPersona(READER, { name: "Wren" });
    const [persona] = listPersonas(READER);

    updatePersona(persona.id, READER, {
    ...
    addChapter(persona.id, READER, "Leaving the agency", "I quit in the spring.");
  ...
  it("says nothing at all when the reader has written no persona", () => {
    expect(readerContext(freshCharacter("persona-silent"), READER)).toBe("");
  });

  it("leaves out the parts that were never written", () => {
    const persona = createPersona(READER, { name: "Bare" });
    const block = readerContext(freshCharacter("persona-bare"), READER);
...
describe("chapters of the reader's life", () => {
  it("keeps them in the order they were added", () => {
    const persona = createPersona(READER, { name: "Ordered" });

    addChapter(persona.id, READER, "One", "First thing.");
    addChapter(persona.id, READER, "Two", "Second thing.");
    const after = addChapter(persona.id, READER, "Three", "Third thing.");

    expect(after?.chapters.map((chapter) => chapter.title)).toEqual(["One", "Two", "Three"]);
  });

  it("rewrites one without disturbing the rest", () => {
    const persona = createPersona(READER, { name: "Edited" });
    addChapter(persona.id, READER, "One", "First thing.");
    const two = addChapter(persona.id, READER, "Two", "Second thing.");
    const target = two?.chapters[1]?.id ?? "";

    const after = updateChapter(target, persona.id, READER, { body: "Rewritten." });
  ...
  it("removes one and leaves the others numbered as they were", () => {
    const persona = createPersona(READER, { name: "Trimmed" });
    addChapter(persona.id, READER, "One", "First thing.");
    const two = addChapter(persona.id, READER, "Two", "Second thing.");
    const target = two?.chapters[0]?.id ?? "";

    const after = deleteChapter(target, persona.id, READER);
